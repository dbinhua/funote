...
function chatlists_new(data,is_none,index){
    var arr = [];
    if (index == 0){
        arr = ['<div class="conv-lists chat-active" id="conv-lists-',data.id,'" style="border-bottom: #eeeeee .5px solid;display:',is_none,'" onclick="selectTarget(\'conv-lists-',data.id,'\',',data.id,')"><div class="list-item conv-item context-menu conv-item-company"><i class="iconfont icon-delete-conv tipper-attached"></i><div class="avatar-wrap"><div class="group-avatar"><div><img class="ui rounded mini image" style="width: 80px;height: 38px" src="../../images/personal.png"></div></div></div><div class="conv-item-content"><div class="title-wrap info"><div class="name-wrap"><p class="name">',data.title,'</p></div><span class="time">上午 10:29</span></div></div></div></div>'];
    }else{
